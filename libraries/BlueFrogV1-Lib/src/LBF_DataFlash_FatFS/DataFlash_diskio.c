...
{
  Stat = STA_NOINIT;
    if ( (FLASH_ReadID()>>16 & 0xFF) == 0x1F )  //check ability to read manufacturer ID
    {
        Stat &= ~STA_NOINIT;
    }
  return Stat;
...
{
//    DRESULT res = RES_OK;
    FLASH_ReadBuffer( (uint8_t*) pBuff, (uint32_t)(sector * BLOCK_SIZE),  (uint32_t)(BLOCK_SIZE * count));  
    return RES_OK;
}
...
//  DRESULT res = RES_ERROR;    
    FLASH_WriteBuffer( (uint8_t*) pBuff, (uint32_t)(sector * BLOCK_SIZE),  (uint32_t)(BLOCK_SIZE * count));  
    return RES_OK;
}
#endif /* _USE_WRITE == 1 */
...
// CTRL_SYNC, GET_SECTOR_SIZE, GET_SECTOR_COUNT, GET_BLOCK_SIZE
  ...
  /* Get number of sectors on the disk (DWORD) */
    case GET_SECTOR_COUNT :
      *(DWORD*)buff = FLASH_CAPACITY / BLOCK_SIZE;
      res = RES_OK;
      break;
