...
static void PWR_EXTI_Init (void);

/* Functions -----------------------------------------------------------------*/

/*******************************************************************************
 * @brief  : Initialise le Power Management LP3913.
 * @param  : Aucun.
 * @return : Rien.
 ******************************************************************************/
void PWR_Init(void)
{
    ...
    PWR_EcrireReg(PWR_BUCK2_ADDR, PWR_BUCK2_VDD);
    
    // Configuration du courant linmit (100mA)
    PWR_EcrireReg(0x20, 0x01);      // 100 mA
    
    // Initialisation de l'interruption ONSTAT :
    PWR_EXTI_Init();
}
...
uint8_t PWR_EcrireReg (uint8_t NomReg, uint8_t ValeurReg)
{
    ...
    // Test sur BUSY Flag
    Tempo(I2C_GetFlagStatus(PWR_I2C, I2C_FLAG_BUSY));
    
    // Envoi du START
    I2C_GenerateSTART(PWR_I2C, ENABLE);
    Tempo(!I2C_CheckEvent(PWR_I2C, I2C_EVENT_MASTER_MODE_SELECT));
    ...
    Tempo(!I2C_CheckEvent(PWR_I2C, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED));
    
    // Envoi de l'adresse du registre
    I2C_SendData(PWR_I2C, NomReg);
    Tempo(!I2C_CheckEvent(PWR_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED));
    
    // Envoi de la donnee
    I2C_SendData(PWR_I2C, ValeurReg);
    Tempo(!I2C_CheckEvent(PWR_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED));
    
    // Envoi du STOP
    I2C_GenerateSTOP(PWR_I2C, ENABLE);
    Tempo(I2C_GetFlagStatus(PWR_I2C, I2C_FLAG_STOPF));
...
uint8_t PWR_LireReg (uint8_t NomReg, uint8_t *ValeurReg)
{
    ...
    *ValeurReg = 0;
    ...
    Tempo(!I2C_CheckEvent(PWR_I2C, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED));
    
    // Desactive l'attente du ACK-Esclave
    I2C_AcknowledgeConfig(PWR_I2C, DISABLE);
    
    // Reception de la donnee
    Tempo(!I2C_CheckEvent(PWR_I2C, I2C_EVENT_MASTER_BYTE_RECEIVED));
    *ValeurReg = I2C_ReceiveData(PWR_I2C);
